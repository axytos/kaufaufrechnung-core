...
class ReportCancelCommand implements AxytosOrderCommandInterface
{
    /**
    ...
    {
        $this->logger->info('Order: ' . $this->order->getOrderNumber() . ' | ReportCancel started');

        try {
            $this->invoiceClient->cancelOrder(new ReportCancelOrderContext($this->order->cancelInformation()));
        ...
        $this->logger->info('Order: ' . $this->order->getOrderNumber() . ' | ReportCancel finished');
    }
}
